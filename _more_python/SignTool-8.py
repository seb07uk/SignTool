...
THEMES = {
    "dark": {
        "bg_base":      QColor(10, 14, 32),
        "orb1":         (30, 80, 220, 55),
        "orb2":         (0, 130, 245, 42),
        "orb3":         (110, 40, 230, 38),
        "orb4":         (0, 190, 210, 22),
        "grid":         QColor(60, 110, 240, 8),
        "vign":         QColor(0, 0, 15, 110),
        "panel_fill":   QColor(18, 28, 65, 210),
        "panel_accent": QColor(22, 45, 110, 230),
        "gloss":        QColor(255, 255, 255, 18),
        "gloss2":       QColor(120, 190, 255, 10),
        "border_top":   QColor(100, 180, 255, 110),
        "border_mid":   QColor(60, 130, 240, 55),
        "border_bot":   QColor(40, 90, 200, 28),
        "inner_glow":   QColor(50, 130, 255, 20),
    },
    "light": {
        "bg_base":      QColor(225, 235, 255),
        "orb1":         (100, 160, 255, 45),
        "orb2":         (60, 180, 245, 35),
        "orb3":         (150, 100, 250, 28),
        "orb4":         (10, 210, 230, 20),
        "grid":         QColor(80, 130, 230, 10),
        "vign":         QColor(170, 195, 240, 70),
        "panel_fill":   QColor(255, 255, 255, 220),
        "panel_accent": QColor(235, 243, 255, 240),
        "gloss":        QColor(255, 255, 255, 140),
        "gloss2":       QColor(210, 230, 255, 80),
        "border_top":   QColor(120, 180, 255, 160),
        "border_mid":   QColor(90, 145, 235, 100),
        "border_bot":   QColor(70, 120, 215, 60),
        "inner_glow":   QColor(110, 170, 255, 25),
    },
}

STYLE_DARK = """
QWidget{font-family:'Segoe UI Semibold','Segoe UI','Arial',sans-serif;font-size:12px;color:#cce4ff;}
QLineEdit{
  background-color:#0d1a40;
  border:1px solid rgba(80,150,255,0.35);
  border-radius:8px;padding:8px 12px;color:#c8e0ff;font-size:12px;
  selection-background-color:rgba(60,140,255,0.5);}
QLineEdit:focus{border:1px solid rgba(100,185,255,0.85);background-color:#0f2050;
  border-bottom:2px solid #4aaeff;}
QLineEdit:hover{border:1px solid rgba(90,160,255,0.55);background-color:#0e1d48;}
QComboBox{
  background-color:#0d1a40;
  border:1px solid rgba(80,150,255,0.35);
  border-radius:8px;padding:8px 12px;color:#c8e0ff;font-size:12px;}
QComboBox:focus{border:1px solid rgba(100,185,255,0.85);}
QComboBox::drop-down{border:none;width:24px;}
QComboBox::down-arrow{image:none;border-left:5px solid transparent;border-right:5px solid transparent;border-top:6px solid rgba(120,185,255,0.8);margin-right:8px;}
QComboBox QAbstractItemView{background-color:#0d1a40;border:1px solid rgba(80,150,255,0.4);color:#b8d8ff;selection-background-color:#1a4db5;outline:none;}
QPushButton{
  background-color:#142060;
  border:1px solid rgba(90,155,255,0.45);
  border-radius:8px;padding:7px 16px;color:#90c4ff;font-size:12px;font-weight:600;
  letter-spacing:0.5px;}
QPushButton:hover{background-color:#1a2e80;border:1px solid rgba(130,200,255,0.7);color:#c8e4ff;}
QPushButton:pressed{background-color:#0f1f60;border-color:rgba(80,150,255,0.5);color:#7ab4ff;}
#ActionButton{
  background:qlineargradient(x1:0,y1:0,x2:0,y2:1,
    stop:0 #2060e8,stop:0.45 #1a50d8,stop:0.55 #1448c8,stop:1 #0e38b0);
  border:1px solid rgba(100,185,255,0.7);border-bottom:2px solid rgba(60,120,220,0.9);
  border-radius:10px;color:#e0f0ff;font-size:13px;font-weight:700;
  letter-spacing:2.5px;padding:12px 24px;}
#ActionButton:hover{
  background:qlineargradient(x1:0,y1:0,x2:0,y2:1,
    stop:0 #3070f5,stop:1 #1a50e0);
  border:1px solid rgba(140,215,255,0.9);color:#ffffff;}
#ActionButton:pressed{background:#1040c0;border-color:rgba(80,150,220,0.6);color:#90c8ff;}
#ActionButton:disabled{background:#0c1840;border:1px solid rgba(50,90,180,0.25);
  color:rgba(100,160,230,0.3);letter-spacing:2.5px;}
QTextEdit{
  background-color:#08102a;
  border:1px solid rgba(50,110,220,0.35);border-left:3px solid rgba(60,140,255,0.5);
  border-radius:8px;color:#5aacff;
  font-family:'Cascadia Code','Consolas','Courier New',monospace;font-size:11px;padding:10px 12px;}
QScrollBar:vertical{background:#0a1530;width:6px;border-radius:3px;}
QScrollBar::handle:vertical{background:rgba(80,150,255,0.45);border-radius:3px;min-height:20px;}
QScrollBar::handle:vertical:hover{background:rgba(110,185,255,0.7);}
QScrollBar::add-line:vertical,QScrollBar::sub-line:vertical{height:0px;}
QScrollBar::add-page:vertical,QScrollBar::sub-page:vertical{background:none;}
QAbstractItemView{background-color:#0d1a40;border:1px solid rgba(80,150,255,0.35);
  color:#b0d4ff;selection-background-color:#1a4db5;font-size:12px;outline:none;}
"""

STYLE_LIGHT = """
QWidget{font-family:'Segoe UI Semibold','Segoe UI','Arial',sans-serif;font-size:12px;color:#1a2d60;}
QLineEdit{
  background-color:#f5f8ff;
  border:1px solid rgba(90,140,220,0.45);
  border-radius:8px;padding:8px 12px;color:#1a2d60;font-size:12px;
  selection-background-color:rgba(80,150,255,0.3);}
QLineEdit:focus{border:1px solid rgba(60,120,230,0.9);background-color:#ffffff;
  border-bottom:2px solid #3a8aff;}
QLineEdit:hover{border:1px solid rgba(80,135,225,0.65);background-color:#f8fbff;}
QComboBox{
  background-color:#f5f8ff;
  border:1px solid rgba(90,140,220,0.45);
  border-radius:8px;padding:8px 12px;color:#1a2d60;font-size:12px;}
QComboBox:focus{border:1px solid rgba(60,120,230,0.9);}
QComboBox::drop-down{border:none;width:24px;}
QComboBox::down-arrow{image:none;border-left:5px solid transparent;border-right:5px solid transparent;border-top:6px solid rgba(60,100,215,0.75);margin-right:8px;}
QComboBox QAbstractItemView{background-color:#f5f8ff;border:1px solid rgba(80,130,220,0.4);color:#1a2d60;selection-background-color:#ccdcff;outline:none;}
QPushButton{
  background-color:#dde8ff;
  border:1px solid rgba(90,140,220,0.5);
  border-radius:8px;padding:7px 16px;color:#1e3a90;font-size:12px;font-weight:600;
  letter-spacing:0.5px;}
QPushButton:hover{background-color:#c8d8ff;border:1px solid rgba(60,115,215,0.8);color:#122870;}
QPushButton:pressed{background-color:#b8cbf5;border-color:rgba(60,110,210,0.6);color:#0a1f58;}
#ActionButton{
  background:qlineargradient(x1:0,y1:0,x2:0,y2:1,
    stop:0 #4488ff,stop:0.45 #3070f0,stop:0.55 #2860e8,stop:1 #1e52d8);
  border:1px solid rgba(80,160,255,0.8);border-bottom:2px solid rgba(30,80,200,0.7);
  border-radius:10px;color:#ffffff;font-size:13px;font-weight:700;
  letter-spacing:2.5px;padding:12px 24px;}
#ActionButton:hover{
  background:qlineargradient(x1:0,y1:0,x2:0,y2:1,
    stop:0 #5598ff,stop:1 #3068f0);
  border:1px solid rgba(110,180,255,0.95);color:#ffffff;}
#ActionButton:pressed{background:#2458d8;border-color:rgba(70,130,220,0.7);color:#ddeeff;}
#ActionButton:disabled{background:#c8d4f0;border:1px solid rgba(90,130,215,0.3);
  color:rgba(80,120,210,0.45);letter-spacing:2.5px;}
QTextEdit{
  background-color:#f0f5ff;
  border:1px solid rgba(90,135,220,0.35);border-left:3px solid rgba(70,140,255,0.5);
  border-radius:8px;color:#1a3a70;
  font-family:'Cascadia Code','Consolas','Courier New',monospace;font-size:11px;padding:10px 12px;}
QScrollBar:vertical{background:#e8eeff;width:6px;border-radius:3px;}
QScrollBar::handle:vertical{background:rgba(90,140,230,0.5);border-radius:3px;min-height:20px;}
QScrollBar::handle:vertical:hover{background:rgba(70,118,215,0.75);}
QScrollBar::add-line:vertical,QScrollBar::sub-line:vertical{height:0px;}
QScrollBar::add-page:vertical,QScrollBar::sub-page:vertical{background:none;}
QAbstractItemView{background-color:#f5f8ff;border:1px solid rgba(80,130,220,0.35);
  color:#1a2d60;selection-background-color:#ccdcff;font-size:12px;outline:none;}
"""
...
class AboutDialog(QDialog):
    ...
    def _apply_theme(self, theme):
        ...
        if is_dark:
            name_c   = "#a8d8ff"
            ver_c    = "rgba(120,190,255,1.0)"
            desc_c   = "rgba(160,200,255,1.0)"
            key_c    = "rgba(120,190,255,1.0)"
            val_c    = "#cce4ff"
            link_c   = "#60b4ff"
            copy_c   = "rgba(120,170,255,1.0)"
            div_c    = "rgba(80,150,255,1.0)"
        else:
            name_c   = "#1a3a80"
            ver_c    = "rgba(50,100,200,1.0)"
            desc_c   = "rgba(30,70,180,1.0)"
            key_c    = "rgba(50,100,200,1.0)"
            val_c    = "#162040"
            link_c   = "#1a60cc"
            copy_c   = "rgba(40,80,180,1.0)"
            div_c    = "rgba(80,130,220,1.0)"
        ...
        btn_style = (
            ...
            f"stop:0 rgba(40,120,255,1.0),stop:1 rgba(10,55,190,1.0));"
            f"border:1px solid rgba(100,190,255,1.0);border-radius:9px;"
            ...
            f"stop:0 rgba(60,150,255,1.0),stop:1 rgba(20,80,210,1.0));"
            f"border:1px solid rgba(140,210,255,1.0);color:#e8f6ff;}}"
            f"QPushButton:pressed{{background:rgba(20,80,210,1.0);color:#7ab4ff;}}"
        ) if is_dark else (
            f"QPushButton{{background:qlineargradient(x1:0,y1:0,x2:0,y2:1,"
            f"stop:0 rgba(70,140,255,1.0),stop:1 rgba(30,80,215,1.0));"
            f"border:1px solid rgba(80,155,255,1.0);border-radius:9px;"
            f"color:#ffffff;font-size:12px;font-weight:bold;letter-spacing:2px;padding:9px 20px;}}"
            f"QPushButton:hover{{background:qlineargradient(x1:0,y1:0,x2:0,y2:1,"
            f"stop:0 rgba(90,160,255,1.0),stop:1 rgba(40,95,230,1.0));"
            f"border:1px solid rgba(110,175,255,1.0);color:#ffffff;}}"
            f"QPushButton:pressed{{background:rgba(40,95,230,1.0);color:#d0e8ff;}}"
        )
...
class CertManagerDialog(QDialog):
    ...
    def _build(self):
        ...
        self._warn_lbl.setStyleSheet(
            "color:rgba(255,180,60,1.0);font-size:10px;background:transparent;")
    ...
    def _apply_theme(self, theme):
        ...
        if is_dark:
            title_c  = "#a8d8ff"
            log_c    = "rgba(120,190,255,1.0)"
            x_style  = ("QPushButton{background:rgba(30,60,150,1.0);border:1px solid rgba(80,140,255,1.0);"
                         "border-radius:5px;color:#80b4ff;font-weight:bold;}"
                         "QPushButton:hover{background:rgba(180,40,40,1.0);color:#ffc8c8;border-color:rgba(255,80,80,1.0);}")
            log_bg   = ("QTextEdit{background:rgba(4,10,30,1.0);border:1px solid rgba(50,120,220,1.0);"
                         "border-radius:7px;color:#5aacff;font-family:'Consolas','Courier New',monospace;"
                         "font-size:11px;padding:9px;}")
        else:
            title_c  = "#1a3a80"
            log_c    = "rgba(40,90,180,1.0)"
            x_style  = ("QPushButton{background:rgba(200,215,255,1.0);border:1px solid rgba(80,130,210,1.0);"
                         "border-radius:5px;color:#2040a0;font-weight:bold;}"
                         "QPushButton:hover{background:rgba(220,60,60,1.0);color:#a01010;border-color:rgba(200,50,50,1.0);}")
            log_bg   = ("QTextEdit{background:rgba(240,247,255,1.0);border:1px solid rgba(90,135,215,1.0);"
        ...
        lbl_color = "rgba(120,190,255,1.0)" if is_dark else "rgba(40,90,180,1.0)"
        for pn in self._panels:
...
class GlassPanel(QFrame):
    ...
    def paintEvent(self, event):
        ...
        if is_dark:
            depth.setColorAt(0,   QColor(255, 255, 255, 6))
            depth.setColorAt(0.4, QColor(0, 0, 0, 0))
            depth.setColorAt(1,   QColor(0, 0, 20, 30))
        else:
            depth.setColorAt(0,   QColor(255, 255, 255, 80))
            depth.setColorAt(0.5, QColor(255, 255, 255, 10))
            depth.setColorAt(1,   QColor(200, 220, 255, 20))
        p.fillPath(path, QBrush(depth))

        # Top gloss band
        gloss_path = QPainterPath()
        gloss_path.addRoundedRect(1, 1, w - 2, h * 0.38, rad - 1, rad - 1)
        gg = QLinearGradient(0, 0, 0, h * 0.38)
        gg.setColorAt(0.0, th["gloss"])
        gg.setColorAt(0.6, th["gloss2"])
        gg.setColorAt(1.0, QColor(255, 255, 255, 0))
        p.fillPath(gloss_path, QBrush(gg))

        # Inner glow at bottom
        ig = QLinearGradient(0, h * 0.65, 0, h)
        ig.setColorAt(0, QColor(0, 0, 0, 0))
        ig.setColorAt(1, th["inner_glow"])
        p.fillPath(path, QBrush(ig))

        # Left accent line (electric blue, subtle)
        if is_dark:
            p.setPen(QPen(QColor(80, 160, 255, 60), 1))
            p.drawLine(1, rad, 1, h - rad)

        # Border gradient — top bright, fades down
        bg = QLinearGradient(0, 0, 0, h)
        bg.setColorAt(0,   th["border_top"])
        bg.setColorAt(0.35, th["border_mid"])
...
class ThemeToggle(QWidget):
    ...
    def paintEvent(self, event):
        ...
        p.fillPath(tp, track)
        p.setPen(QPen(QColor(120, 180, 255, 255), 1))
        p.drawPath(tp)
        ...
        if self._is_light:
            glow.setColorAt(0, QColor(255, 230, 100, 255))
        else:
            glow.setColorAt(0, QColor(100, 160, 255, 255))
        glow.setColorAt(1, QColor(0, 0, 0, 255))
...
class LangButton(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if is_dark:
            p.fillPath(path, QColor(40, 90, 200, 255))
            p.setPen(QPen(QColor(100, 170, 255, 255), 1))
        else:
            p.fillPath(path, QColor(180, 210, 255, 255))
            p.setPen(QPen(QColor(80, 130, 220, 255), 1))
        p.drawPath(path)
        ...
        if is_dark:
            p.fillPath(hl, QColor(80, 160, 255, 255))
        else:
            p.fillPath(hl, QColor(80, 140, 220, 255))
...
class TabBar(QWidget):
    ...
    def _restyle(self):
        if self._theme == "dark":
            on  = ("QPushButton{background:qlineargradient(x1:0,y1:0,x2:0,y2:1,"
                   "stop:0 #1e4acc,stop:1 #152fa0);"
                   "border:1px solid rgba(100,175,255,0.65);border-bottom:2px solid #4aaeff;"
                   "border-radius:7px;color:#deeeff;font-weight:700;padding:6px 18px;font-size:12px;"
                   "letter-spacing:0.5px;}")
            off = ("QPushButton{background:#0e1a3e;border:1px solid rgba(70,120,220,0.3);"
                   "border-radius:7px;color:rgba(140,185,255,0.55);padding:6px 18px;font-size:12px;}"
                   "QPushButton:hover{background:#142260;border:1px solid rgba(100,165,255,0.5);"
                   "color:rgba(180,215,255,0.85);}")
        else:
            on  = ("QPushButton{background:qlineargradient(x1:0,y1:0,x2:0,y2:1,"
                   "stop:0 #4080ff,stop:1 #2860e8);"
                   "border:1px solid rgba(70,140,255,0.75);border-bottom:2px solid #2050d0;"
                   "border-radius:7px;color:#ffffff;font-weight:700;padding:6px 18px;font-size:12px;"
                   "letter-spacing:0.5px;}")
            off = ("QPushButton{background:#dde6ff;border:1px solid rgba(90,135,215,0.4);"
                   "border-radius:7px;color:rgba(50,90,180,0.65);padding:6px 18px;font-size:12px;}"
                   "QPushButton:hover{background:#ccd8ff;border:1px solid rgba(70,120,210,0.65);"
                   "color:rgba(30,65,160,0.9);}")
...
class AeroBackground(QWidget):
    ...
    def paintEvent(self, event):
        ...
        def orb(cx, cy, rad, rgba, sharp=0.0):
            ...
            if sharp > 0:
                gr.setColorAt(sharp,  QColor(r, g, b, int(a * 0.55)))
            gr.setColorAt(1,          QColor(r // 6, g // 6, b // 6, 0))
            p.setBrush(QBrush(gr))
            p.setPen(Qt.PenStyle.NoPen)
            p.drawEllipse(int(cx - rad), int(cy - rad), rad * 2, rad * 2)

        o = self._off
        # 4 large animated orbs
        orb(w * .25 + math.sin(math.radians(o * .65)) * 60,
            h * .20 + math.cos(math.radians(o * .50)) * 30, 280, th["orb1"], 0.3)
        orb(w * .80 + math.cos(math.radians(o * .55)) * 50,
            h * .65 + math.sin(math.radians(o * .75)) * 40, 220, th["orb2"], 0.28)
        orb(w * .10 + math.sin(math.radians(o * .38 + 90)) * 40,
            h * .78 + math.cos(math.radians(o * .42)) * 25, 190, th["orb3"], 0.22)
        orb(w * .90 + math.cos(math.radians(o * .45 + 45)) * 32,
            h * .15 + math.sin(math.radians(o * .60)) * 24, 155, th["orb4"])
        # 2 small accent orbs for depth
        orb(w * .50 + math.sin(math.radians(o * .30 + 120)) * 80,
            h * .50 + math.cos(math.radians(o * .35)) * 35, 120,
            (th["orb1"][0], th["orb1"][1], th["orb1"][2], th["orb1"][3] // 2))
        orb(w * .65 + math.cos(math.radians(o * .28 + 60)) * 45,
            h * .35 + math.sin(math.radians(o * .32 + 30)) * 28, 95,
            (th["orb3"][0], th["orb3"][1], th["orb3"][2], th["orb3"][3] // 2))
        ...
        if is_dark:
            ...
            for _ in range(70):
                sx = rng.randint(0, w)
                sy = rng.randint(0, h)
                pulse = int(20 + 20 * math.sin(math.radians(o * 2.1 + sx * 0.5)))
                p.setBrush(QColor(190, 215, 255, pulse))
                sz = rng.choice([1, 1, 1, 1, 2])
                p.drawEllipse(sx, sy, sz, sz)

        # Subtle dot grid
        gc = th["grid"]
        dot_a = min(gc.alpha() * 3, 28)
        p.setBrush(QColor(gc.red(), gc.green(), gc.blue(), dot_a))
        p.setPen(Qt.PenStyle.NoPen)
        step = 44
        ...
        # Faint grid lines
        line_a = min(gc.alpha(), 12)
        p.setPen(QPen(QColor(gc.red(), gc.green(), gc.blue(), line_a), 1))
        for x in range(0, w, step): p.drawLine(x, 0, x, h)
        for y in range(0, h, step): p.drawLine(0, y, w, y)

        # Subtle inner edge glow (dark only)
        if is_dark:
            p.setBrush(Qt.BrushStyle.NoBrush)
            p.setPen(QPen(QColor(60, 130, 255, 22), 12))
            p.drawRect(0, 0, w, h)

        # Bottom vignette
        vgn = QLinearGradient(0, h * .50, 0, h)
        vgn.setColorAt(0, QColor(0, 0, 0, 0))
        vgn.setColorAt(1, th["vign"])
        p.setBrush(QBrush(vgn)); p.setPen(Qt.PenStyle.NoPen)
        p.drawRect(0, int(h * .50), w, h)

        # Top vignette (dark only)
        if is_dark:
            tvgn = QLinearGradient(0, 0, 0, h * .16)
            tvgn.setColorAt(0, QColor(0, 0, 12, 80))
            tvgn.setColorAt(1, QColor(0, 0, 0, 0))
            p.setBrush(QBrush(tvgn))
            p.drawRect(0, 0, w, int(h * .16))
...
class _AboutButton(QPushButton):
    ...
    def paintEvent(self, _event):
        ...
        if self._hovered:
            ...
            glow.setColorAt(0, QColor(80, 180, 255, 255) if is_dark else QColor(60, 130, 230, 255))
            glow.setColorAt(1, QColor(0, 0, 0, 255))
            p.setBrush(QBrush(glow))
            p.setPen(Qt.PenStyle.NoPen)
        ...
        if self._pressed:
            bg_grad.setColorAt(0, QColor(20, 80, 200, 255))
            bg_grad.setColorAt(1, QColor(10, 50, 160, 255))
        elif self._hovered:
            if is_dark:
                bg_grad.setColorAt(0, QColor(60, 160, 255, 255))
                bg_grad.setColorAt(1, QColor(20, 90, 210, 255))
            else:
                bg_grad.setColorAt(0, QColor(80, 150, 255, 255))
                bg_grad.setColorAt(1, QColor(40, 100, 220, 255))
        else:
            if is_dark:
                bg_grad.setColorAt(0, QColor(40, 110, 220, 255))
                bg_grad.setColorAt(1, QColor(15, 60, 180, 255))
            else:
                bg_grad.setColorAt(0, QColor(100, 160, 240, 255))
                bg_grad.setColorAt(1, QColor(60, 110, 210, 255))
        ...
        gg.setColorAt(1, QColor(255, 255, 255, 255))
        ...
        # "i" glyph — dot + stem
        glyph_col = QColor(220, 240, 255, 255) if is_dark else QColor(255, 255, 255, 255)
        p.setPen(Qt.PenStyle.NoPen)
...
class SignToolGUI(AeroBackground):
    ...
    def _build_ui(self):
        ...
        self._sub_lbl.setStyleSheet("color:rgba(140,190,255,1.0);font-size:10px;letter-spacing:2px;background:transparent;")
        ...
        self._theme_lbl.setStyleSheet("color:rgba(140,190,255,1.0);font-size:10px;letter-spacing:1px;background:transparent;")
    ...
    def _page_cert_mgr(self):
        ...
        self._cert_tab_lbl.setStyleSheet(
            "font-size:12px;color:rgba(160,200,255,1.0);background:transparent;")
    ...
    def _refresh_label_styles(self, is_light):
        if is_light:
            ...
            ss = "color:rgba(40,80,180,1.0);font-size:10px;letter-spacing:2px;background:transparent;"
            ls = "color:rgba(40,90,180,1.0);font-size:10px;font-weight:bold;letter-spacing:2px;background:transparent;"
            ws = "color:rgba(160,100,0,1.0);font-size:11px;background:transparent;"
            ths= "color:rgba(40,90,180,1.0);font-size:10px;letter-spacing:1px;background:transparent;"
        else:
            ts = "color:#a8d8ff;font-size:16px;font-weight:bold;letter-spacing:2px;background:transparent;"
            ss = "color:rgba(140,190,255,1.0);font-size:10px;letter-spacing:2px;background:transparent;"
            ls = "color:rgba(120,190,255,1.0);font-size:10px;font-weight:bold;letter-spacing:2px;background:transparent;"
            ws = "color:rgba(255,200,80,1.0);font-size:11px;background:transparent;"
            ths= "color:rgba(140,190,255,1.0);font-size:10px;letter-spacing:1px;background:transparent;"
    ...
    def _slbl_raw(self):
        lbl = QLabel()
        lbl.setStyleSheet("color:rgba(120,190,255,1.0);font-size:10px;font-weight:bold;letter-spacing:2px;background:transparent;")
    ...
    def _restyle_opt(self, btn):
        ...
        if btn.isChecked():
            if is_light:
                btn.setStyleSheet("QPushButton{background:rgba(60,120,220,1.0);border:1px solid rgba(60,120,220,1.0);border-radius:6px;color:#1a3a80;font-weight:bold;padding:6px 14px;}")
            else:
                btn.setStyleSheet("QPushButton{background:rgba(40,120,220,1.0);border:1px solid rgba(120,200,255,1.0);border-radius:6px;color:#cce8ff;font-weight:bold;padding:6px 14px;}")
        else:
            if is_light:
                btn.setStyleSheet("QPushButton{background:rgba(255,255,255,1.0);border:1px solid rgba(80,130,200,1.0);border-radius:6px;color:rgba(60,100,180,1.0);padding:6px 14px;}"
                                  "QPushButton:hover{background:rgba(255,255,255,1.0);color:#2a4a90;}")
            else:
                btn.setStyleSheet("QPushButton{background:rgba(255,255,255,1.0);border:1px solid rgba(100,160,255,1.0);border-radius:6px;color:rgba(160,200,255,1.0);padding:6px 14px;}"
                                  "QPushButton:hover{background:rgba(80,140,220,1.0);color:#a0ccff;}")
